...
class PCA(object):
    ...
    def _eigen_decomp(self, X):
        ...
        e_vecs = np.array([i for _,i in sorted(zip(e_vals,e_vecs.T), reverse=True)])
        e_vecs = e_vecs.T
        e_vals = np.array(sorted(e_vals, reverse=True))
        # END YOUR CODE
    ...
    def transform(self, X, n_components):
        ...
        #self.fit(X)
        X_centered = X - self.mean
        X_proj = np.matmul(X_centered, self.W_pca[:,:n_components])
        # END YOUR CODE
    ...
    def reconstruct(self, X_proj):
        ...
        X_centered = np.matmul(X_proj, self.W_pca[:,:n_components].T)
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        e_vals = [i.real for i in e_vals]
        e_vecs = np.array([i for _,i in sorted(zip(e_vals,e_vecs), reverse=True)])
        e_vals = np.array(sorted(e_vals, reverse=True))
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            X_i = np.array([X[j] for j in range(y.shape[0]) if y[j] == i])
            mu_i = X_i.mean(axis = 0)
            for j in range(len(X_i)):
                X_centered = X_i[j] - mu_i
                covariance = np.outer(X_centered.T, X_centered)
                scatter_within += covariance
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        X_prime = X.copy()
        for i in np.unique(y):
            idx = [j for j in range(y.shape[0]) if y[j] == i]
            mean = np.array([X_prime[j] for j in idx]).mean(axis = 0)
            for j in idx:
                X_prime[j] = mean
        X_centered = X_prime - mu
        scatter_between = np.dot(X_centered.T, X_centered)
        # END YOUR CODE
