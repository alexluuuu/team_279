...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        closest_y = y_train[sorted_ind[:k]]
...
def split_folds(X_train, y_train, num_folds):
    ...
    for i in range(num_folds):
        X_trains[i] = np.concatenate([X_split[j] for j in range(num_folds) if j != i])
        y_trains[i] = np.concatenate([Y_split[j] for j in range(num_folds) if j != i])
        X_vals[i] = X_split[i]
        y_vals[i] = Y_split[i]
    # END YOUR CODE
